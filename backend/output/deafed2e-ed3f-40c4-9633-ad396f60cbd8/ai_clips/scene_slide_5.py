...
class SlideScene_5(Scene):
    def construct(self):
        ...
        title = Text("Summary: The Three Laws", font_size=48)
        title.to_edge(UP)
        self.play(Write(title), run_time=1)

        bullets = VGroup()
        b0 = Text("Law 1: Objects resist changes in motion", font_size=36)
        b0.next_to(title, DOWN, buff=1.0)
        bullets.add(b0)
        b1 = Text("Law 2: Force is tied to mass and acceleration", font_size=36)
        b1.next_to(title, DOWN, buff=1.6)
        bullets.add(b1)
        b2 = Text("Law 3: Forces always come in pairs", font_size=36)
