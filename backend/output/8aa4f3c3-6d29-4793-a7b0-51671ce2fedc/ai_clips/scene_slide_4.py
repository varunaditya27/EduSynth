...
class SlideScene_4(Scene):
    def construct(self):
        ...
        title = Text("Law 3: Action and Reaction", font_size=48)
        title.to_edge(UP)
        ...
        bullets.add(b1)
        b2 = Text("Forces always occur in pairs", font_size=36)
        b2.next_to(title, DOWN, buff=2.2)
        bullets.add(b2)

        self.wait(0.2)
        self.play(FadeIn(b0), run_time=2.9728)
        self.wait(0.2)
        self.play(FadeIn(b1), run_time=2.9728)
        self.wait(0.2)
        self.play(FadeIn(b2), run_time=2.9728)
        self.wait(0.5)
